...
public:
    ...
    成功时`rsz`返回结果：
        ...
        0：文件结束
    */
    ::lom::Err::Ptr Read(char *buf, ssize_t sz, ssize_t &rsz, int64_t timeout_ms = -1) const;
    ...
    */
    ::lom::Err::Ptr Write(const char *buf, ssize_t sz, ssize_t &wsz, int64_t timeout_ms = -1) const;

    //写数据，成功则保证写完，`sz`必须>=0，`sz`为0时仅做有效性检查
    ::lom::Err::Ptr WriteAll(const char *buf, ssize_t sz, int64_t timeout_ms = -1) const;
...
向地址`ip:port`建立TCP连接，IPV4版本
    ...
    `timeout_ms`指定超时时间
*/
::lom::Err::Ptr ConnectTCP(const char *ipv4, uint16_t port, Conn &conn, int64_t timeout_ms = -1);
...
*/
::lom::Err::Ptr ConnectUnixSockStream(const char *path, Conn &conn, int64_t timeout_ms = -1);
...
*/
::lom::Err::Ptr ConnectUnixSockStreamWithAbstractPath(const Str &path, Conn &conn, int64_t timeout_ms = -1);
