...
需要注意大部分使用迭代器的场景下是从当前位置开始，不断判断有效、获取数据并向后`Inc`，所以如果传递rend边界，
...
public:
    ...
    /*
    前进若干位置
        - `step`为负数则表示后退（如果具体实现支持的话），`step`为0则不改变状态
        - 若迭代器不支持后退，传入负数的话行为未定义
        - 在边界处执行`Inc`或一次`step`过长越过了边界，行为由各实现定义，一般建议越过边界时保留在边界处，
          并在下次反向`Inc`时回到有效位置
    */
...
/*
带`Size`方法的Iterator
`Size`返回从当前位置开始，通过`Valid()`判断和`Inc()`调用会顺序迭代出的元素数量，正确性由实现者保证
*/
template<typename T>
