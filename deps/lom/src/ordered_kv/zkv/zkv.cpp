...
void DBImpl::WriteZM(const WriteBatch::RawOpsMap &wb_ops, ZMap *old_zm)
{
    static const ssize_t
        kZLDataLenMax = 1024,
        kZLDataLenMergeThreshold = kZLDataLenMax / 2;

    //`Write`调用这个方法时对`write_lock_`加了锁，已经保证了没有人修改`zm_`，所以不加`update_lock_`直接读
    auto zm = zm_;
    if (old_zm)
    {
        *old_zm = zm;
